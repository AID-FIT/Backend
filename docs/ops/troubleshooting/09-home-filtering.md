...
`_HOME_TILE_COUNT`를 36으로 올리는 것이 가장 단순한 답으로 보였다. 그런데
`llm_service`를 따라가 보면 그 숫자가 프롬프트 크기를 직접 결정한다.

```python
candidate_limit = max(MAX_LLM_CANDIDATES, max_recommendations * 2)
```

36을 요구하면 **후보 72건을 프롬프트에 싣고, 이유 36개를 생성**해야 한다. 입력도
출력도 4배가 된다. "칩당 13초"를 "로딩당 25초"로 바꾸는 것에 지나지 않는다.

> 개수를 늘리는 상수가 **어디까지 전파되는지** 먼저 따라간다. 이 상수는 타일 수가
> 아니라 LLM 프롬프트 크기였다.
